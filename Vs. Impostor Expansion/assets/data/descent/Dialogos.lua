...
function onCreatePost() 
  if not seenCutscene and isStoryMode then
   ...
   setProperty('white.alpha',0.3)
    ...
    addAnimationByPrefix('bfDialogue', 'realization', 'BF_realization', 24, false);
    ...
    setProperty('fakeTxt.antialiasing',true)
      ...
      playMusic('dialogue/nothing', 1,true)    
end
end
...
if dialogue == 2 and (getMouseX('camHUD') > 0 and getMouseX('camHUD') < 1280) and (getMouseY('camHUD') > 0 and getMouseY('camHUD') < 720 and mouseClicked('left')) or dialogue == 3 and keyJustPressed('accept') then
...
function onTimerCompleted(tag)
    ...
    bfTalk = false   
    anim = 'realization'
    texto="Why is it so dark in here..."
